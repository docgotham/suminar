...
export function createSuminarMcpServer(service: ConversationService, extensions: SuminarMcpExtensions = {}): McpServer {
  ...
  server.registerTool("suminar_sync_conversation", {
    ...
    inputSchema: {
      ...
      completedVisibleEventsCopiedWithoutOmission: z.array(z.object(syncableEventShape)).min(1).max(500).describe("Every completed visible user or host contribution after afterCursor, in exact conversational order — all of them, omitting none. Usually that is just the previous host message and the current user message, but if visible turns have accumulated since the last synchronization (the user conversed with you before addressing an agent again), backfill every one of those intervening turns too, oldest first — including your own substantive replies: analyses and syntheses you authored are seminar speech, not private work product."),
