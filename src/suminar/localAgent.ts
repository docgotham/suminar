...
export class OpenAiAnswerGenerator implements AnswerGenerator {
  ...
  async generate(
    ...
    const instructions = [
      ...
      titleStyleClause
        ? `When naming the represented work in prose, use its registered title exactly, ${titleStyleClause}: ${styledTitle}. In parenthetical page citations, use its registered short title with the same styling — (${styledShortTitle}, p. 12)${shortWorkTitle !== source.title ? " — rather than the full title" : ""}. Style it identically in every parenthetical citation in the answer, not only the first${source.workType === "standalone" ? ", with the asterisks tight against the short title and no spaces inside them" : ""}. Do not present a chapter title, running-header title, or an abbreviation of your own devising as the work's title.`
