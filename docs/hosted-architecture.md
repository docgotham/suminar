...
- Upload API (`/api/documents`, `src/hosted/documents.ts`): auth required; accepts PDF/.docx up to 256 MB; writes original to Storage (`{owner}/originals/{id}.{ext}`); inserts `documents` row (`uploaded`); processes inline for the pilot (the `documents` status enum already models the async swap for later).
