...
- Status transitions written to `documents`; `needs_ocr_review` surfaces the Mistral OCR retry as an authenticated action. **As built (v1.0.27):** the extraction function also flags **glyph corruption** (broken ToUnicode ligature maps — symbols/digits inside words like "Poli)cs" at densities real prose never has: ≥15 hits and ≥0.4% of words upgrade clean → `partial_needs_ocr_review`), and gains an `ocr: true` mode calling the Mistral OCR REST API (`mistral-ocr-latest`) on the signed source URL — rendered-page reading, immune to a corrupt embedded text layer; requires `MISTRAL_API_KEY`, and the source travels to Mistral for that user-triggered call. `POST /documents/:id/ocr` (metered under uploads) re-extracts, rebuilds the agent in place (same agent id — it derives from the source hash), preserves card metadata, and the dashboard's "Retry with OCR" button (shown only for `needs_ocr_review` documents) chains auto-identify on the clean text. A failed OCR retry restores the document's prior status instead of marking it failed.
